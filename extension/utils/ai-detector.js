...
// Pre-compiled patterns for offline heuristic detection (compiled once at module load)

// Common AI transition phrases for text analysis
...
// Cross-line patterns require multiline mode
const AI_MULTILINE_PATTERNS = [
...
// Known AI image generation service domains
...
// Keywords indicating AI-generated images in alt text
const AI_ALT_KEYWORDS = [
  'generated by ai', 'ai generated', 'midjourney', 'stable diffusion',
  'dall-e', 'ai art', 'generated with', 'created by ai', 'ai图片', 'AI生成'
];

// Common dimension pairs produced by AI image generators
const AI_IMAGE_DIMENSIONS = [
...
function analyzeTextHeuristics(text) {
  ...
  // Check for common AI transition phrases using pre-compiled patterns
  let phraseMatches = 0;
  AI_PHRASE_PATTERNS.forEach(pattern => {
    if (pattern.test(text)) phraseMatches++;
  });
  AI_MULTILINE_PATTERNS.forEach(pattern => {
    if (pattern.test(text)) phraseMatches++;
  });
...
function analyzeImageHeuristics(imageData) {
  ...
  if (src) {
    ...
    for (const domain of AI_IMAGE_DOMAINS) {
  ...
  if (alt) {
    ...
    for (const kw of AI_ALT_KEYWORDS) {
  ...
  if (width && height) {
    for (const [w, h] of AI_IMAGE_DIMENSIONS) {
      if (width === w && height === h) {
        score += 15;
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { detectTextAI, detectImageAI, analyzeTextHeuristics, analyzeImageHeuristics, getScoreLabel };
}
